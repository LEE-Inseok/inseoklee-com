---
conference: 한국 군사기술학회 (Korea Military Technology Society)
title: Selective CO-to-Hydrocarbon Conversion in Formic Acid-Promoted Sodium Borohydride Hydrolysis for High-Purity Hydrogen Production
date: "2026-06"
type: oral
status: presented
images:
  - /recognition/kmts-2026/venue.jpg
  - /recognition/kmts-2026/slide-title.jpg
  - /recognition/kmts-2026/slide-summary.jpg
  - /recognition/kmts-2026/slide-conclusion.jpg
summary: Oral presentation delivered at the Korea Military Technology Society 2026 meeting. Reframes the SBH-CO selectivity work for defense logistics — on-demand high-purity hydrogen generation from chemical hydride hydrolysis suited to field-portable and remote power systems.
---

## Topic

Same body of catalytic-selectivity work as the KHNES poster, presented with a defense-applications framing:

- On-demand H₂ supply without high-pressure storage
- Built-in CO mitigation removing a downstream subsystem in the field unit
- Portability and robustness considerations for mobile / remote power

## Why this venue

KMTS is the natural domestic venue for the defense-applications track of the SBH hydrogen system — same audience that earlier evaluated the Defense R&D Idea Competition entry.
